...
class Colony:
    ...
    def __init__(self, bacteria_type, colony_size, chance_resistant, horizontal_gene_transfer, reproduction_time, drug_survival_chance, _num_initial_resistant):
        ...
        for i in range(0,self._num_initial_resistant):
            new_cell = Cell(resistant=True, horizontal_transmission=False, death_probability_rate=.1, reproduction_probability_rate=.15)
        ...
        for i in range(0,self._colony_size - self._num_initial_resistant):
            if random.randint(0,self._initial_resistance_chance) == 2:
                ...
                new_cell = Cell(resistant=False, horizontal_transmission=False, death_probability_rate=.1, reproduction_probability_rate=.15)
            self.cells.append(new_cell)
